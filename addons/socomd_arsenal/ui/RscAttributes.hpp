...
class GVAR(socomd_display) {
    idd = IDD_ace_arsenal;
    enableSimulation=1;
    onLoad = QUOTE([ARR_3('onLoad', _this, QQGVAR(socomd_display))] call FUNC(onArsenalOpen));
    onUnload = QUOTE([ARR_3('onUnload', _this, QQGVAR(socomd_display))] call FUNC(onArsenalClose));
    onKeyDown = QUOTE([ARR_3('onKeyDown', _this, QQGVAR(socomd_display))] call FUNC(onKeyDown));
    onKeyUp = QUOTE(GVAR(shiftState) = _this select 2);
    onMouseButtonDown = QUOTE([ARR_3('onMouseButtonDown', _this, QQGVAR(socomd_display))] call FUNC(onMouseButtonDown));
    onMouseButtonUp = QUOTE([ARR_3('onMouseButtonUp', _this, QQGVAR(socomd_display))] call FUNC(onMouseButtonUp));
    ...
    class ControlsBackground {
        ...
        class mouseArea: ctrlStatic {
            idc = IDC_mouseArea;
            style = 16;
            onMouseMoving = QUOTE([ARR_3('onMouseMoving', _this, GVAR(socomd_display))] call FUNC(handleMouse));
            onMouseHolding = QUOTE([ARR_3('onMouseHolding', _this, GVAR(socomd_display))] call FUNC(handleMouse));
            onMouseZChanged = QUOTE([ARR_3('onMouseZChanged', _this, GVAR(socomd_display))] call FUNC(handleScrollWheel));
            x = QUOTE(safezoneX);
            y = QUOTE(safezoneY);
            w = QUOTE(safezoneW);
            h = QUOTE(safezoneH);
