...
switch (GVAR(currentLeftPanel)) do {
    ...
    case IDC_buttonNVG : {
        ...
        _nvgArray = ["SOCOMD_NVG", "SOCOMD_NVG_GR", "SOCOMD_NVG_AM", "SOCOMD_NVG_B", "SOCOMD_NVG_GR_B", "SOCOMD_NVG_AM_B", "SOCOMD_NVG_C", "SOCOMD_NVG_GR_C", "SOCOMD_NVG_AM_C", "SOCOMD_NVG_GPNVG_WP_black", "SOCOMD_NVG_GPNVG_GR_black"];
        ...
        if ((GVAR(currentItems) select 8) != _item) then {
            ...
            // else update all socomd NVG items in inventory
                ...
                GVAR(center) setUnitLoadout _unitLoadout;
            };
        };
        call FUNC(showItem);
        TOGGLE_RIGHT_PANEL_HIDE
    ...
    case IDC_buttonGrenadeLoadout : {
        if (_item != "") then {
            [GVAR(center)] call FUNC(removeGrenades);
            [GVAR(center),_item] call FUNC(addSelection);
            GVAR(center) setVariable  ["ace_socomd_arseal_grenade", _item];
        };
        // call FUNC(showItem);
        TOGGLE_RIGHT_PANEL_HIDE
        [_display, _control, _curSel, (configFile >> "CfgWeapons" >> _item)] call FUNC(itemInfo);
    ...
    case IDC_buttonMap : {
        ...
        GVAR(center) setVariable  ["ace_socomd_arseal_extras", _item];
        TOGGLE_RIGHT_PANEL_HIDE
        [_display, _control, _curSel, (configFile >> "CfgWeapons" >> _item)] call FUNC(itemInfo);
    };
